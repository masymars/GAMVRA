...
import { FileText, ImageIcon, FileAudio, X, Upload, Save, Loader, Bot, Eye, Camera } from 'lucide-react';
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-black/40 via-black/60 to-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 p-8 max-w-3xl w-full max-h-[90vh] flex flex-col transform transition-all duration-300 scale-100 relative">
          ...
          <div className="flex-shrink-0 mb-6">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">Edit Medical Record</h2>
            <p className="text-gray-600 mt-2">Update your medical record information</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 -mr-6 scrollbar-thin scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 scrollbar-track-gray-100 scrollbar-thumb-rounded-full">
            <div className="space-y-4 pr-4">
              ...
              {recordToEdit.files && recordToEdit.files.length > 0 && (
                <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-4">
                  <div className="flex justify-between items-center mb-3">
                      <h3 className="text-sm font-semibold text-gray-700 flex items-center">
                        ...
                        Uploaded Files ({recordToEdit.files.length})
                      </h3>
                      <button
                      ...
                      >
                          {isAnalyzing || isLoading ? <Loader className="w-4 h-4 animate-spin" /> : <Bot className="w-4 h-4" />}
                          <span>{isAnalyzing || isLoading ? 'Analyzing...' : 'Analyze & Fill Form'}</span>
                      </button>
                  </div>
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-200">
                    {recordToEdit.files.map((file) => (
              ...
              <div className="border-t border-gray-200 pt-6 mt-6 space-y-4">
                  <div className="flex items-center space-x-2 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-gray-800">Record Details</h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => setNewRecord({...recordToEdit, title: e.target.value})} 
                    ...
                    <div>
                      ...
                      <input 
                        type="date" 
                        value={recordToEdit.fileDate} 
                        onChange={(e) => setNewRecord({...recordToEdit, fileDate: e.target.value})} 
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-colors bg-white text-gray-900" 
                        required
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      onChange={(e) => setNewRecord({...recordToEdit, category: e.target.value})} 
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-colors bg-white text-gray-900"
                    >
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      onChange={(e) => setNewRecord({...recordToEdit, notes: e.target.value})} 
                      ...
                      <div className="p-4 bg-gradient-to-br from-teal-50 to-cyan-50 rounded-lg text-sm text-gray-700 whitespace-pre-line border border-teal-100 min-h-[120px] shadow-sm">
                          {isAnalyzing ? (
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-teal-600">Generating analysis...</span>
                            </div>
                          ) : analysis || (
                            <span className="text-gray-500 italic">Analysis will appear here after you upload files and click 'Analyze & Fill Form'.</span>
                          )}
                      </div>
                  </div>
              </div>
          ...
          <div className="flex-shrink-0 flex justify-end space-x-3 pt-6 border-t border-gray-200 mt-6">
            ...
            <button 
              ...
              disabled={!recordToEdit.title || !recordToEdit.fileDate || isAnalyzing || isLoading} 
